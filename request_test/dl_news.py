...
class Down_info(object):
    def __init__(self):
        ...
        self.path_video = "/home/data/videos/tengxun/"
        self.path_pic = "/home/data/images/tengxun/"
        self.path_smpic = "/home/data/thumb/tengxun/"
        # self.path_video = "/Users/chenfei/desktop/ceshi/vid/"  # TODO 上传前修改
        # self.path_pic = "/Users/chenfei/desktop/ceshi/pic/"  # TODO 上传前修改
        # self.path_smpic = "/Users/chenfei/desktop/ceshi/smpic/"
...
class Handle(object):
    ...
    def down_file(self):
        ...
        if len(item["videoAddr"]) > 0 or len(item["imageAddr"]) > 0:
            try:
                ...
                if video_info:
                    con = requests.get(video_info, headers=headers, timeout=5)
                    if con.status_code == 200:  # 如果请求成功
                        ...
                        v_pic = requests.get(item["videoPic"], headers=headers, timeout=5)
                        item["videoPic"], url = self.do.save_pic(v_pic.content, item["videoPic"])

                        if pic_info:
                            temp_path = ""
                            for i in pic_info:
                                con = requests.get(i, headers=headers, timeout=5)
                                path, url = self.do.save_pic(con.content, i)
                                temp_path += path + ","
                                item["content"] = re.sub(i, path, item["content"])

                            item["content"] = "<video src='" + video_path + "'" + " poster='" + item[
                                "videoPic"] + "'></video>" + item["content"]
    ...
    def handle_excpt(self, item):
        try:
            if item:
                item["content"] = re.sub("jpg1", "jpg", item["content"])

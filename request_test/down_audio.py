...
REDIS_URL ='redis://auth:nFP1$w0zrL7v@192.168.116.173'
class Down_info(object):
    ...
    def get_audio_redis(self):
        a = self.cli.lpop("kaola")  # TODO 表名待定
        if a:  # 返回字典
            # return demjson.decode(a.decode())
            return eval(a)
    ...
    #     path = "/data/videos/tengxun/" + url.split("/")[-1][:10] + str(int(time.time())) + ".mp4"
...
class Handle(object):
    def __init__(self):
        ...
        # self.table = self.connection.table(b'audio_test')
        self.table = self.connection.table(b'tempcf1')
...
#     temp_c = Handle()
#     while True:
#         i = temp_c.do.get_redis_num("kaola")
#         if i > 0:
